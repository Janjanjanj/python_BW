# -*- coding: utf-8 -*-
"""
Created on Thu Sep 15 09:04:37 2022
...
def main():
    def open():
        # strassennamen
        strassenname = '1_010_Karlsruhe_Rinth_Querallee'
        clusterklein = '1_010'
       
        # open the csv file were data is stored
        csv_alt = pd.read_csv('V:/_Projekte/22-037_Bundesanstalt_Bundesimmobilien_Sued_Aussenanlagen/01_Vorgaben/Python_Vorlagen/BW_tabelle_nur_flur.csv', sep = ';')
        ...
        return flaeche, linie, haus, mieter, csv_alt, path_flaeche, strassenname
        ...
        def mietergaerten():
            ...
            if value.all():
                ...
                clip_flaeche = gpd.clip(flaeche_diss, mieter_diss)
                
    
                df1 = flaeche_diss.overlay(clip_flaeche, how='symmetric_difference')
                ...
                flaeche_diss = flaeche.dissolve(by = 'Layer')
                haus_diss = haus.dissolve()
            return flaeche_diss, haus_diss, value
            ...
            def leere_liste():
                ...
                csv = csv_alt[csv_alt["Flurstnr"].isin(liste_flurnames)].reset_index(drop=True)
                csv['IR_Gebaeude'].fillna(0, inplace=True)
                return flurnames_all, csv 
                ...
                def loop():
                    ...
                    for i in range(len(csv)):
                       ...
                       # two lists: first is the area of each Layer and second Layername  
                        ...
                        if value.all():
                           ...
                           bigdata.loc[-1] = ["Mietergaerten", 0]  # adding a row
                           bigdata.index = bigdata.index + 1  # shifting index
                           bigdata = bigdata.sort_index()  # sorting by index
                           
                           bigdata[bigdata['Layer']=='Mietergaerten'].index[0]
                           bigdata['a'][bigdata[bigdata['Layer']=='Mietergaerten'].index[0]] = mieter.area
                       ...
                       # this loops through for one flurstueck and adds the fitting flache areas to the csv  
                        ...
                        #subs.to_excel('V:/_Projekte/22-037_Bundesanstalt_Bundesimmobilien_Sued_Aussenanlagen/04_Auswertung/04_Auswertung_Tabellen/BW/'+strassenname+'.xlsx', sheet_name='sheet1', index=False)
                    ...
                    def merge():
                       ...
                       def linie():
                           ...
                           merged['VF_oef_Gehweg_flaeche'] = test['length']*test['width']
                           return merged
                       return merged
