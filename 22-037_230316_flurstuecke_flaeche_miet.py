...
# strassennamen
strassenname = '0_003_Heroldstatt_Neue_Siedlung'
clusterklein = '0_003'
...
flur1 = gpd.read_file(path_flur + 'alle_flurstuecke_stand_220804.gpkg')
...
if len(value)>=1:
     ...
     #flur1 = flur1.dissolve()
     #clip_flaeche = gpd.clip(flur1, mieter_diss, keep_geom_type=True)
     mieter_buff = mieter_diss['geometry'].buffer(1000)
     mieter_buff_diff = mieter_buff.difference(mieter_diss)
     df2 = gpd.clip(flaeche, mieter_buff_diff,keep_geom_type=True)
     #df2 = df2[df2.geometry.type != 'LineString']
     df2 = df2.dissolve(by = "Layer")
     #df2.to_file('C:/Users/JKraemer/Documents/loschen/MyGeometries24.shp', driver='ESRI Shapefile')
     flaeche_diss = df2
...
###### part for the mietergarten
miet = gpd.read_file(path_flaeche+'flaeche/Mietergaerten.shp')
miet = miet.to_crs(epsg=25832)
miet_diss = miet.dissolve(by = 'Layer')
miet = gpd.overlay(miet_diss, flur, how="intersection")
miet['miet'] = miet.area
#test = pd.merge(merged,linie, on = ['width'] how = 'outer')
test = pd.merge(merged,miet, how = 'outer')
miet.columns
test = pd.merge(merged, miet, how ='outer')
merged['Mietergaerten'] = test['miet']
...
linie = gpd.overlay(linie_diss, flur, how="intersection")
